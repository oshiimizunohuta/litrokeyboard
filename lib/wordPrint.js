...
WordPrint.prototype ={
	...
	isHorizon8: function()
	{
		return (this.chipSize == 8 && this.soundmarkAlign == 'horizon');
	},
	
	isVertical8: function()
	{
		return (this.chipSize == 8 && this.soundmarkAlign == 'vertical');
	},
	...
	makeStrId: function(MS)
	{
		var i, TheWord, baseword = 0, subword = this.SPACE_CODE
			, baseWords = []//行の文字
			, subWords = []//上段行の文字
			, isHorizon8 =  this.isHorizon8(), isSoundmark
			;
		...
		for(i = 0; i < MS.length; i++){
			...
			if(baseword == -1){
				...
				subWords = this.escCommand(MS, i, subWords);
				i += 1;
				continue;
		}
			
			if(!isHorizon8){subWords.push(subword);}
			baseWords.push(baseword);
			
			//改行ポイント
			if((baseWords.length % this.cols) == 0 && (this.cols > 0)){
				if(!isHorizon8){this.newLine(subWords);}
				this.newLine(baseWords);
				baseWords = [];
				subWords = [];
			}
			//横1段型
			if(isHorizon8 && isSoundmark){
				//位置を記憶
				this.soundmarkPos.push({line:this.wordIds.length, left:baseWords.length});
				baseWords.push(subword);
				// subWords.push(this.SPACE_CODE);
				//改行ポイント
				if((baseWords.length % this.cols) == 0 && (this.cols > 0)){
					// this.newLine(subWords);
					this.newLine(baseWords);
					baseWords = [];
					subWords = [];
				}
	...
	{
		var ar, spr, i, j, isHorizon8 = this.isHorizon8();
		this.wordIds = [];//行
		this.soundmarkPos = [];
		...
		if(isHorizon8){
			for(i = 0; i < this.soundmarkPos.length; i++){
				this.spriteArray[this.soundmarkPos[i].line][this.soundmarkPos[i].left].rot(2);
			}
		}
	},
	...
	{
		var i, spr, ofs = 0
		, isHorizon8 = this.isHorizon8()
		, isVertical8 = this.isVertical8()
		, ofy = isVertical8 ? -this.chipSize : 0
		;
		...
		for(i = 0; i < this.spriteArray.length; i++){
			lineSpace = isVertical8 ? (((i / 2) | 0) * this.rowSpace) + (i * this.chipSize) : i * (this.rowSpace + this.chipSize);
