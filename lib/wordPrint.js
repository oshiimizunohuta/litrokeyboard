...
WordPrint.prototype ={
	...
	makeStrId: function(MS)
	{
		var i, TheWord, baseword = 0, subword = this.SPACE_CODE
			, baseWords = []//行の文字
			, subWords = []//上段行の文字
			, isHorizon =  this.isHorizon(), isSoundmark
			;
		...
		for(i = 0; i < MS.length; i++){
			...
			if(!isHorizon){subWords.push(subword);}
			baseWords.push(baseword);
	...
	{
		var ar, spr, i, j, isHorizon = this.isHorizon();
		this.wordIds = [];//行
		this.soundmarkPos = [];
	...
	draw: function(scr)
	{
		...
		for(i = 0; i < this.spriteArray.length; i++){
			lineSpace = isVertical ? (((i / 2) | 0) * this.rowSpace) + (i * this.chipSize) : i * (this.rowSpace + this.chipSize);
