...
//		from.h = this.canvas.height;
	...
	{//echo(x + " " + y +  "   ");
		...
		if(sprite.rotFlag){
			r = sprite.rotFlag;
			r = (((1 == r) * 90) + ((2 == r) * 180) + ((3 == r) * 240)) * Math.PI() / 180;
			this.ctx.rotate(r);
		}
...
imageResource.makePointSprite = function(name, index, size)
{
	...
	return new CanvasSprite(name, x, y, size, size);
...
imageResource.makeSprite = function(name, index)
{
	...
	return new CanvasSprite(name, x, y, this.separateWidth[name], this.separateHeight[name]);
};
...
imageResource.makeSpriteArray = function(name, indexes)
{
	...
	for(var i = 0; i < indexes.length; i++){
		...
		spriteArray.push(new CanvasSprite(name, x, y, this.separateWidth[name], this.separateHeight[name]));
	}
...
imageResource.makeSpriteChunk = function(name, indexes)
{
	...
	for(var j = 0; j < indexes.length; j++){
		...
		for(var i = 0; i < indexes[0].length; i++){
			var x = (indexes[j][i] % cellsX) * this.separateWidth[name];
			var y = (0 | (indexes[j][i] / cellsX)) * this.separateHeight[name];
			spriteChunk[j].push(new CanvasSprite(name, x, y, this.separateWidth[name], this.separateHeight[name]));
		}
	}
	return spriteChunk;
};
 ...
 * @returns
 */
function CanvasSprite(name, x, y, w, h)
{
	...
	CanvasSprite.prototype.rot = rot;
	...
	CanvasSprite.prototype.swapStart = swapStart;
	/**
	 * 色を交換
	 */
