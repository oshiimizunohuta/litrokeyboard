...
function keyHold(){
	...
	Object.keys(allcontrolls).forEach(function(idname){
		...
		Object.keys(c.name2code).forEach(function(name){
			c.holdon(c.name2code[name]);
		});
	});
	
	// for(idname in allcontrolls){
		// for(name in allcontrolls[idname].name2code){
		// }
	// }
}
/**
...
function keyUntrig(){
	...
	// for(idname in allcontrolls){
		// for(name in allcontrolls[idname].name2code){
			// allcontrolls[idname].untrig(allcontrolls[idname].name2code[name]);
		// }
	// }
	var name, c;
	Object.keys(allcontrolls).forEach(function(idname){
	...
	function getState(name)
	{
		if(typeof name == "object"){
			...
			name.forEach(function(n, i){
				states[n] = this.controlls[n].state;
			}, this);
			return states;
		}else{
