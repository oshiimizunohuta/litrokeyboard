...
function LitroKeyboard() {
	...
	this.paramKeys = ['TYPE', 'VOLUME', 'ATTACK', 'DECAY', 'SUSTAIN', 'LENGTH', 'RELEASE' , 'V-SPD', 'V-DPT', 'V-RISE', 'V-PHS', 'SWEEP', 'DELAY', 'DETUNE', ];
	...
	this.eventsetMenuList = ['NOTEOFF', 'RESTART', 'RETURN', ];
...
LitroKeyboard.prototype = {
	...
	onCode: function(chr)
	{
		...
		this.litroSound.onNoteFromCode(channel, code, octave, paramChannel);
		...
		//仮使用
		if(channel == paramChannel){
			this.setEventChange(paramChannel, this.makeEventset('note', code + (octave * this.octaveInKeys)));
		}
	...
	channelMove: function(dir)
	{
		...
		;
		switch(dir){
			case 'up': this.litroSound.toggleOutput(cur.x, true);
							this.drawChannelTab_on(cur.x, true)
							break;
			case 'down': this.litroSound.toggleOutput(cur.x, false);
							this.drawChannelTab_on(cur.x, false)
							break;
	...
	moveCatchCursor: function(dir, ext)
	{
		...
		switch(dir){
			...
			case 'down': this.catchType = catchKey[(catchVal + 1) % catchKey.length];
							break;
			case 'left':
							eventset = this.player.searchNearBack(ch, this.player.noteSeekTime, 0, this.catchType, ignore);
							break;
		...
		if(eventset != null){
			if(this.catchType != 'ALL' && ((this.catchType == 'TUNE' && this.selectNote.type == 'note') || (this.catchType == 'note' && this.selectNote.type != 'note')) && (this.selectNote.ch != ch || this.selectNote.type != this.catchType)){
				//違うチャンネルをキャッチした
				this.initCatchEvent();
				this.initSelect();
			}
	...
	moveCharBoardCursor: function(dir, ext)
	{
		...
		if(ext){
			if(tlength == 1){return;}
			switch(dir){
				// case 'up': Ccur.y = (Ccur.y + Climit.y - 3) % Climit.y;
						// curr.y = curr.y - 2; break;
				// case 'down': Ccur.y = (Ccur.y + 2) % Climit.y;
						// curr.y = curr.y + 2; break;
				case 'left': Tcur.x = (Tcur.x + tlength - 1) % tlength; break;
				case 'right': Tcur.x = (Tcur.x + 1) % tlength; break;
			}			
	...
	baseKeyOnChannel: function(key, ext)
	{
		...
		switch(key){
			case '<': 
				...
				break;
			case '>': 
				this.setEventChange(cur.x, this.makeEventset(param, this.litroSound.getChannel(cur.x, param, false)));
				// this.litroSound.setChannel(cur.x, param, this.litroSound.getChannel(cur.x, param) + 1);
				// this.drawParamCursor();
				break;
			case 'select': 
	...
	drawCharBoard: function()
	{
		...
		for(x = 0; x < this.player.titleMaxLength; x++){
			chrNum = word.SearchNum(this.player.title.substr(x, 1));
//			rev = this.fileTitleCursor.x == x;
			y = 0;
			if(x >= this.player.title.length){
				tspr = setsprite(word.SPACE_CODE, rev);
				spr = setsprite(word.HYPHEN_CODE, rev);
				y = cellhto(0.25);
			}else if(chrNum in word.soundmarks){
				tspr = setsprite(chrNum, rev);
				spr = setsprite(word.soundmarks[chrNum], rev);
			}else{
				tspr = setsprite(word.SPACE_CODE, rev);
				spr = setsprite(chrNum, rev);
			}
			scr.drawSprite(tspr, cellhto(x + cm.x), cellhto(tcm.y) + y);
			scr.drawSprite(spr, cellhto(x + cm.x), cellhto(tcm.y + 1) + y);
		}
